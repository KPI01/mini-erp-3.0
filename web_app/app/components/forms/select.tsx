...
export default function SelectInput({ name, options, state, config, errors }: SelectInputProps) {
    ...
    const [display] = useState(state.value)

    return (
        <Grid columns="1" gapY="1" align="center" className={config?.containerClass}>
            {config?.label && <Label.Root>{config.label}</Label.Root>}
            <Select.Root name={name} size="3" value={state.value} onValueChange={state.changer} disabled={keys.length < 1}>
                <Select.Trigger variant="surface" placeholder={keys.length < 1 ? "Nada que mostrar..." : ""} >
                    {display && options[display]}
                </Select.Trigger>
                <Select.Content position="popper">
                    {keys.map((k, i) => {
                        return <Select.Item key={i} value={k} >
                            {options[k]}
                        </Select.Item>
                    })}
                </Select.Content>
